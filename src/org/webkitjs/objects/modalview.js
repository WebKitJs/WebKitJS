...
webkitjs.ModalView = function(comp, state) {
	...
	var closebutton = new webkitjs.SimpleButton('X', 'CLOSED');
...
webkitjs.ModalView.prototype.handleState = function (sState) {
	var oThis = this;
	this.getStateController().register('WIZARD', function() {
		oThis.show();
	}, function() {
		oThis.hide();
	});
...
webkitjs.ModalView.prototype.show = function () {
	for ( var c in this.data_) {
		this.data_[c].component.getElement().css({zIndex: webkitjs.ModalView.lastZ_++});
	}
